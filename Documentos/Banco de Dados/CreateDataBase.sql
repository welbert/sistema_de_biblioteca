...
CREATE TABLE IF NOT EXISTS `mydb`.`tus_tipo_usuario` (
  `tus_tp_usario` INT NOT NULL AUTO_INCREMENT,
  `tus_ds_usuario` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`tus_tp_usario`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`usr_usuario` (
  ...
  INDEX `fk_usr_usuario_tus_tipo_usuario_idx` (`usr_tp_usario` ASC),
  CONSTRAINT `fk_usr_usuario_tus_tipo_usuario`
    FOREIGN KEY (`usr_tp_usario`)
    REFERENCES `mydb`.`tus_tipo_usuario` (`tus_tp_usario`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
