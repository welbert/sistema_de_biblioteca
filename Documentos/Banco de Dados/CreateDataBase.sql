...
DELIMITER $$
USE `mydb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`TAI_alu_aluno` AFTER INSERT ON `alu_aluno` FOR EACH ROW
BEGIN
update usr_usuario
...
USE `mydb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`TBD_alu_aluno` BEFORE DELETE ON `alu_aluno` FOR EACH ROW
BEGIN
...
USE `mydb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`TAI_pro_professor` AFTER INSERT ON `pro_professor` FOR EACH ROW
BEGIN
update usr_usuario
set usr_ch_inativo = 'N'
...
USE `mydb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`TBD_pro_professor` BEFORE DELETE ON `pro_professor` FOR EACH ROW
BEGIN
